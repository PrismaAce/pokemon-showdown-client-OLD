...
class BattleTooltips {
	...
	showMoveTooltip(move: Move, isZOrMax: string, pokemon: Pokemon, serverPokemon: ServerPokemon, gmaxMove?: Move) {
		...
		if (this.battle.hardcoreMode) {
			...
			if (move.flags.cutting && ability === 'sharpenedblade') {
				text += `<p class="movetag">&#x2713; Cutting <small>(boosted by Sharpened Blade)</small></p>`;
			}
			if (move.flags.kicking && ability === 'steeltoes') {
				text += `<p class="movetag">&#x2713; Kick <small>(boosted by Steel Toes)</small></p>`;
			}
			if ((move.flags.pulse || move.flags.bullet) && ability === 'megalauncher') {
				text += `<p class="movetag">&#x2713; Pulse / Ballistic <small>(boosted by Mega Launcher)</small></p>`;
			}
			...
			if (move.flags.bullet || move.flags.pulse) {
				text += `<p class="movetag">&#x2713; Pulse / Ballistic <small>(doesn't affect Bulletproof pokemon)</small></p>`;
			}
		}
		return text;
	}
	...
	calculateModifiedStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon) {
		...
		if (weather) {
			...
			if (ability === 'leafguard' && (weather === 'sunnyday' || weather === 'desolateland')) {
				stats.def = Math.floor(stats.def * 1.5);
				stats.spd = Math.floor(stats.spd * 1.5);
			}
		...
		if (ability === 'infernalheart' && serverPokemon.hp <= serverPokemon.maxhp / 4) {
			if (serverPokemon.hp > serverPokemon.maxhp * 0.9) {
				stats.atk = Math.floor(stats.atk * 1.3);
				stats.spa = Math.floor(stats.spa * 1.3);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.8 && serverPokemon.hp <= serverPokemon.maxhp * 0.9) {
			
				stats.atk = Math.floor(stats.atk * 1.32);
				stats.spa = Math.floor(stats.spa * 1.32);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.7 && serverPokemon.hp <= serverPokemon.maxhp * 0.8) {
				stats.atk = Math.floor(stats.atk * 1.36);
				stats.spa = Math.floor(stats.spa * 1.36);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.6 && serverPokemon.hp <= serverPokemon.maxhp * 0.7) {
				stats.atk = Math.floor(stats.atk * 1.42);
				stats.spa = Math.floor(stats.spa * 1.42);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.5 && serverPokemon.hp <= serverPokemon.maxhp * 0.6) {
				stats.atk = Math.floor(stats.atk * 1.5);
				stats.spa = Math.floor(stats.spa * 1.5);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.4 && serverPokemon.hp <= serverPokemon.maxhp * 0.5) {
				stats.atk = Math.floor(stats.atk * 1.61);
				stats.spa = Math.floor(stats.spa * 1.61);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.3 && serverPokemon.hp <= serverPokemon.maxhp * 0.4) {
				stats.atk = Math.floor(stats.atk * 1.74);
				stats.spa = Math.floor(stats.spa * 1.74);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.2 && serverPokemon.hp <= serverPokemon.maxhp * 0.3) {
				stats.atk = Math.floor(stats.atk * 1.89);
				stats.spa = Math.floor(stats.spa * 1.89);
			}
			if (serverPokemon.hp > serverPokemon.maxhp * 0.1 && serverPokemon.hp <= serverPokemon.maxhp * 0.2) {
				stats.atk = Math.floor(stats.atk * 2.06);
				stats.spa = Math.floor(stats.spa * 2.06);
			}
			if (serverPokemon.hp <= serverPokemon.maxhp * 0.1) {
				stats.atk = Math.floor(stats.atk * 2.25);
				stats.spa = Math.floor(stats.spa * 2.25);
			}
		}
		if (clientPokemon) {
	...
	getMoveType(move: Move, value: ModifiableValue, forMaxMove?: boolean | Move): [TypeName, 'Physical' | 'Special' | 'Status'] {
		...
		if (allowTypeOverride && category !== 'Status' && !move.isZ && !move.id.startsWith('hiddenpower')) {
			if (moveType === 'Normal') {
				...
				if (value.abilityModify(0, 'Fear Forge')) moveType = 'Ghost';
				if (value.abilityModify(0, 'Iron Forge')) moveType = 'Steel';
			}
	...
	getMoveAccuracy(move: Move, value: ModifiableValue, target?: Pokemon) {
		...
		} else if (value.tryAbility('Steel Toes') && move.flags.kicking) {
			accuracyModifiers.push(5325);
	...
	getMoveBasePower(move: Move, moveType: TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (
			...
			if (move.type === 'Normal') {
				...
				value.abilityModify(this.battle.gen > 6 ? 1.2 : 1.3, "Fear Forge");
				value.abilityModify(this.battle.gen > 6 ? 1.2 : 1.3, "Iron Forge");
		...
		if (move.flags['cutting']) {
			value.abilityModify(1.33, 'Sharpened Blade');
		}
		if (move.flags['steeltoes']) {
			value.abilityModify(1.3, 'Steel Toes');
		}
